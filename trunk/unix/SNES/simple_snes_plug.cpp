...
int rateHeader(const u8* data,const u32 size,const u32 addr)
{
  if(size < addr + 64) return 0;  //image too small to contain header at this location?
  int score = 0;

  u16 resetvector = data[addr + ResetVector] | (data[addr + ResetVector + 1] << 8);
  u16 checksum    = data[addr + Checksum   ] | (data[addr + Checksum    + 1] << 8);
  u16 complement  = data[addr + Complement ] | (data[addr + Complement  + 1] << 8);

  u8 resetop = data[(addr & ~0x7fff) | (resetvector & 0x7fff)];  //first opcode executed upon reset
  u8 mapper  = data[addr + Mapper] & ~0x10;                      //mask off irrelevent FastROM-capable bit
...
u32 getHeaderOffset(const u8* data,const u32 size)
{
	s32 lo = rateHeader(data, size, 0x007fc0);
	s32 hi = rateHeader(data, size, 0x00ffc0);
	s32 ex = rateHeader(data, size, 0x40ffc0);
...
/*bool preCacheBuffer(const u8* buf,const int size,const std::string& filename)
{
...
{
	return preCacheBuffer((const u8*)SMP_PLAYER_BUFFER,sizeof(SMP_PLAYER_BUFFER)/sizeof(SMP_PLAYER_BUFFER[0]),"s-smp_player.bin");
}

static void fixSPCBuffer(NeoResultBlock* block)
{
	static u8 spc_sh[0x100];
	...
	memcpy((u8*)spc_sh,(const u8*)block->romBuffer,0x100);
	memmove(((u8*)block->romBuffer),((const u8*)block->romBuffer)+0x100,0x10080);
	memmove(((u8*)block->romBuffer) + 0x10080,(const u8*)spc_sh,0x100);
}*/

bool isExtendedFormat(NeoResultBlock* block,const u8* md5) /*extended formats go here (vgm/spc/custom/hacks)*/
{	
	/*
	const u8* data = (const u8*)block->romBuffer;
	const u32 size = (const u32)block->romBufferLength;
	u8* p;

	static const u8 spcSignature[] = 
	{ //"SNES-SPC700"
		0x53,
		0x4e,
		0x45,
	...
	//bad
	static const u8 vgzSignature[] =//{1F 8B} 08 08 9F 3E CE 4A 02
	{ 
		0x1F, 
		0x8B,
	};
	...
	if(memcmp(data,vgmSignature,sizeof(vgmSignature) / sizeof(vgmSignature[0])) == 0)
	{
		...
		if(generateSMPPlayerBinary())
		{
			...
			if(block->romBufferLength < 0x10080 + 0x100)
			{
				...
				p = new u8[block->romBufferLength];
...
void configureImage(NeoResultBlock* block,const u8* md5)
{
	...
	const u8* data = (const u8*)block->romBuffer;
	const u32 size = (const u32)block->romBufferLength;
	const u32 headOffs = getHeaderOffset(data,size);
	...
	const u8 mapperid = data[headOffs+Mapper];
	const u8 rom_type = data[headOffs+RomType];
	const u8 rom_size = data[headOffs+RomSize];
	const u8 company  = data[headOffs+Company];
	const u8 regionid = data[headOffs+CartRegion] & 0x7f;
	const s32 sram = 1024 << (data[headOffs+RamSize] & 7);
	...
	*/
	...
	//Align to 4Mb
	{
		if(force4Mb)
		{
			const u32 newSize = 4 * 131072;
			u8* p = (u8*)block->romBuffer;
			u8* p2 = new u8[newSize];
			...
			if(p2)
			{
				...
				block->romBuffer = (u8*)p2;
			}
		}
	}
}

bool snes_plug_detect(NeoResultBlock* p_block,const bool rm_header)
{
	if(rm_header)
	{
		if((p_block->romBufferLength & 0x3FF) != 0)
		{
			u8* p = (u8*)p_block->romBuffer;

			p_block->romBufferLength -= 0x200;

			if((s32)p_block->romBufferLength < 1)
				return false;

			memmove(p,p + 0x200,p_block->romBufferLength); 
			memset(p + p_block->romBufferLength,'\0',0x200);
		}
	}

	configureImage(p_block,0);
	return true;//well..
}
